  ...
  return (
    <div className="rise-in flex items-center gap-3 rounded-2xl border border-border bg-surface/40 shadow-sm px-4 py-2.5">
      <div className="flex items-center gap-2 shrink-0">
        ...
        <span className="text-[12px] font-semibold text-text-muted uppercase tracking-wider whitespace-nowrap">
          Выбранные пожары
        </span>
        <span className="text-[12px] text-accent tabular-nums font-medium">{selected.length}</span>
      </div>
      ...
      <div className="flex-1 min-w-0 flex items-center gap-2 overflow-x-auto scrollbar-thin py-0.5">
        {selected.map((f) => (
          ...
          >
            <span className="text-[13px] font-mono font-semibold text-warning tabular-nums">#{f.fire_id}</span>
            <span className="flex flex-col leading-tight">
              <span className="text-[12px] text-text tabular-nums">{fmtDate(f.dt_first)}</span>
              <span className="text-[10px] text-text-dim tabular-nums">{Math.round(f.Area).toLocaleString("ru-RU")} га</span>
            </span>
            <button
              onClick={() => onToggle(f.fire_id)}
              className="text-text-dim hover:text-danger transition-colors text-[15px] leading-none w-4 h-4 flex items-center justify-center rounded focus-ring"
      ...
      <button
        onClick={onClear}
        className="shrink-0 text-[11px] text-text-dim hover:text-danger transition-colors focus-ring rounded px-1"
      >
        сбросить
      </button>
    </div>
